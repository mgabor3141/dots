---
description: Review recent changes for bugs, missed requirements, and loose ends
---
Review all changes on the current branch or all relevant commits.
...
1. Get the full diff and commit log:
   - **jj**: `jj diff -r 'trunk()..@'` and `jj log -r 'trunk()..@'`
   - **git**: `git diff origin/main..HEAD` and `git log --oneline origin/main..HEAD`
   - If on trunk with no branch, review the last commits that are relevant to this thread.
...
4. Check for:
   ...
   - Check for things that could be improved or simplified, either now or in a followup task in the near future
   - Check the commit history: verify that it contains atomic commits that are clear for reviewers to follow
