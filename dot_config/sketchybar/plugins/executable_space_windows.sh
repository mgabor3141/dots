...
if [ "$SENDER" = "aerospace_node_moved" ]; then
    apps=$(aerospace list-windows --workspace "$TARGET_WORKSPACE" | awk -F'|' '{gsub(/^ *| *$/, "", $2); print $2}')
    sketchybar --set space.$TARGET_WORKSPACE drawing=on
    icon_strip=" "
    if [ "${apps}" != "" ]; then
    while read -r app; do
        icon_strip+=" $($CONFIG_DIR/plugins/icon_map_fn.sh "$app")"
    ...
    sketchybar --set space.$TARGET_WORKSPACE label="$icon_strip"
fi
