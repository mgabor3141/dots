...
The PNG is generated by a chezmoi `run_onchange_` script (`run_onchange_generate_led_image.sh`) using ImageMagick: three composited layers (outer glow → mid glow → bright core) with gaussian blur, matching the waybar mute indicator's CSS gradient style (`dot_config/waybar-mute-indicator/style.css`). The script only re-runs when its content changes.

A `modify_` script was tried first but doesn't work for ImageMagick-generated images: the blur operation produces non-deterministic output across process invocations (ASLR affects floating-point rounding), so `chezmoi diff` always shows changes even immediately after `chezmoi apply`.
